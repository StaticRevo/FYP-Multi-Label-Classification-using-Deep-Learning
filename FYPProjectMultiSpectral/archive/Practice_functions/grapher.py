...
dot = Digraph(
    comment='Custom WideResNetB4-ECA Model',
    format='png',
    graph_attr={
        'rankdir': 'TB',
        'splines': 'false',  
        'bgcolor': '#f0f0f0',
        'pad': '0.5',
        'label': 'Custom WideResNetB4-ECA Architecture',
        'labelloc': 't',
        'fontsize': '18',
        'fontcolor': '#333333',
    },
...
dot.node('Conv1', 'Conv1 (3x3, 38 filters)\nBatchNorm2d\nReLU\n[B, 38, 120, 120]', fillcolor='#cce5ff')
...
# WideBasicBlockECA subgraph with curved skip connection
with dot.subgraph(name='cluster_WideBlock') as wb:
    ...
    wb.edge('WB_Skip_Dummy', 'WB_Skip', style='dashed', color='#ff3333', penwidth='2')

# ECA subgraph with curved skip-like connection
with dot.subgraph(name='cluster_ECA') as eca:
    ...
    eca.node('ECA_Reshape2', 'Reshape\n[B, C, 1, 1]', fillcolor='#ffe6cc')  # Added
    eca.node('ECA_Sigmoid', 'Sigmoid', fillcolor='#ffccb3')  # Added
    eca.node('ECA_Skip_Dummy', '', shape='point', width='0.01', style='invisible')
    eca.node('ECA_Skip', 'Input Reuse', style='filled', fillcolor='#ffcccc')  # Relabeled
    eca.node('ECA_Mul', '×\n[B, C, H, W]', shape='circle', fillcolor='#ffffff', width='0.5')  # Added
...
dot.node('Layer2', 'Layer2\n1 WideBasicBlockECA\n63 filters, stride=2\n[B, 63, 60, 60]', fillcolor='#b3d9ff')  # 1 block
dot.node('Layer3', 'Layer3\n1 WideBasicBlockECA\n126 filters, stride=2\n[B, 126, 30, 30]', fillcolor='#b3d9ff')  # 1 block
...
dot.node('Block3', 'Block 3\nDepthwiseSeparableConv(96->96, stride=2)\nBatchNorm2d\nGELU\nWideBottleneck(96->176)\nSE\nDropout\n[B, 176, 15, 15]', fillcolor='#b3d9ff')
dot.node('SkipAdapter', 'Skip Adapter\nConv2d(48->176, 1x1)\nAvgPool2d(4x4)\n[B, 176, 15, 15]', fillcolor='#b3d9ff')
dot.node('Fusion', '+', shape='circle', fillcolor='#ffcccc', width='0.7')
